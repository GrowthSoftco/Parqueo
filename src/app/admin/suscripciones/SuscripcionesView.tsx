...
const estadoStyle: Record<string, React.CSSProperties> = {
  'Al día': { background: '#0f2a1a', color: '#22c55e', border: '1px solid #1a4a2a' },
          ...
          <tbody>
            {rows.map(r => (
              ...
              >
                ...
                <td className="px-5 py-3.5" style={{ color: '#aaa', fontSize: '13px' }}>{r.plan}</td>
                <td className="px-5 py-3.5 text-white" style={{ fontSize: '13px', fontWeight: 600 }}>{r.monto}</td>
                <td className="px-5 py-3.5" style={{ color: '#888', fontSize: '13px' }}>{r.prox}</td>
      ...
      {row && (
        <>
          ...
          <div
            ...
            style={{ width: 420, maxWidth: '92vw', background: '#0f0f0f', borderLeft: '1px solid #1e1e1e', transform: open ? 'translateX(0)' : 'translateX(100%)', transition: 'transform 300ms cubic-bezier(0.16, 1, 0.3, 1)' }}
          >
    ...
    <div>
      <div className="flex items-start justify-between p-5 border-b" style={{ borderColor: '#1c1c1c' }}>
        <div>
          ...
          <p style={{ color: '#666', fontSize: '12px' }}>Facturación de la cuenta</p>
        </div>
      ...
      <div className="p-5 flex flex-col gap-6">
        <div className="p-4 rounded-xl" style={{ background: '#141414', border: '1px solid #1e1e1e' }}>
          <div className="flex items-center justify-between mb-2">
            ...
            <span className="text-white" style={{ fontSize: '15px', fontWeight: 700 }}>{row.monto}<span style={{ color: '#666', fontWeight: 400, fontSize: '12px' }}>/mes</span></span>
          </div>
          <div className="flex items-center gap-2" style={{ color: '#888', fontSize: '12px' }}>
        ...
        <div>
          <p style={{ color: '#666', fontSize: '11px', fontWeight: 600, letterSpacing: '0.05em', marginBottom: '10px' }}>CAMBIAR PLAN</p>
            ...
            {['Básico', 'Pro', 'Negocio'].map(l => {
              ...
              return (
                <button
                  ...
                  style={{ background: on ? 'rgba(139,92,246,0.1)' : '#141414', border: on ? '1px solid #8b5cf6' : '1px solid #1e1e1e', color: on ? '#fff' : '#bbb', fontSize: '13px', fontWeight: 600 }}
                >
                  {l}
                </button>
              )
          ...
          <p style={{ color: '#666', fontSize: '11px', fontWeight: 600, letterSpacing: '0.05em', marginBottom: '12px' }}>ACCIONES</p>
          <div className="flex flex-col gap-2">
            <button onClick={() => act(() => marcarPagado(row.id))} className="flex items-center gap-3 px-3.5 py-2.5 rounded-lg w-full text-left transition-colors" style={{ background: '#141414', border: '1px solid #1e1e1e', color: '#ccc' }} onMouseEnter={e => (e.currentTarget.style.background = '#1a1a1a')} onMouseLeave={e => (e.currentTarget.style.background = '#141414')}>
              <RefreshCw size={15} /> <span style={{ fontSize: '13px' }}>Marcar como pagado</span>
            </button>
            {row.estado === 'Suspendido' || row.estado === 'Baneado' ? (
              <button onClick={() => act(() => cambiarEstadoCuenta(row.id, 'ACTIVE'))} className="flex items-center gap-3 px-3.5 py-2.5 rounded-lg w-full text-left transition-colors" style={{ background: '#141414', border: '1px solid #1e1e1e', color: '#22c55e' }} onMouseEnter={e => (e.currentTarget.style.background = '#1a1a1a')} onMouseLeave={e => (e.currentTarget.style.background = '#141414')}>
                <CheckCircle2 size={15} /> <span style={{ fontSize: '13px' }}>Reactivar cuenta</span>
              </button>
            ) : (
              <button onClick={() => act(() => cambiarEstadoCuenta(row.id, 'SUSPENDED'))} className="flex items-center gap-3 px-3.5 py-2.5 rounded-lg w-full text-left transition-colors" style={{ background: '#141414', border: '1px solid #1e1e1e', color: '#ef4444' }} onMouseEnter={e => (e.currentTarget.style.background = '#1a1a1a')} onMouseLeave={e => (e.currentTarget.style.background = '#141414')}>
