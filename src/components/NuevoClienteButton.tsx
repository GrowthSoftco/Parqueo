          ...
          <div className="rounded-2xl w-full max-w-sm p-6" style={{ background: 'var(--c-surface)', border: '1px solid var(--c-border3)' }} onClick={e => e.stopPropagation()}>
            ...
            <div className="flex flex-col gap-3">
              <div className="flex flex-col gap-1.5">
                <label style={{ color: 'var(--c-text3)', fontSize: '13px' }}>Nombre</label>
              ...
              <div className="flex flex-col gap-1.5">
                <label style={{ color: 'var(--c-text3)', fontSize: '13px' }}>Email (opcional)</label>
                <input value={email} onChange={e => setEmail(e.target.value)} placeholder="correo@ejemplo.com" style={inputStyle} />
              </div>
              {error && <p style={{ color: '#ef4444', fontSize: '13px' }}>{error}</p>}
              <button onClick={submit} disabled={pending} className="w-full rounded-full py-2.5 mt-2 text-black font-semibold" style={{ background: 'var(--c-accent)', fontSize: '14px', opacity: pending ? 0.6 : 1 }}>
